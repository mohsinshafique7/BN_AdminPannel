...
const SourceCategoriesList = (props) => {
  ...
  const onSendForm = (values) => {
    props.createSourceCategory(values).catch(() => openNotification("error"));
  };
  ...
  const tableData = (item) => (
    <>
      <Link className="item-link" to={`/source-category/${[item.id]}`}>
        {[item.name]}
      </Link>
  ...
  return (
    <>
      ...
      <Search />
      <CoreForm title={"Create Source Category"} inputData={inputData} onSendForm={onSendForm} />
